...
<html>
<head>
<TITLE>AdvancedSearchLocal Result</TITLE>
...
<%  int showingResult = (Integer)request.getAttribute("showingResult"); 
	...
	<form action="AdvancedSearchServlet" method="POST">
		<p>field:<span class="red"><%=field %></span>&nbsp;&nbsp;searchMethod:<%=searchMethod %></p>
		
		<input type="hidden" name="start" value="<%=start %>" />
		<input type="hidden" name="end" value="<%=end %>" />
		<input type="hidden" name="searchMethod" value="<%=searchMethod %>" size="7">
		<input type="hidden" name="field" value="<%=field %>" size="7">
	...
	<% List<String> fieldContentList = (ArrayList<String>) request.getAttribute("fieldContentList"); %>
	...
	<span class="red"><%=fieldContentList.get(i) %></span><br/>
	<%
		}
	%>
	<!-- 显示结束 -->
