...
//		System.out.println(highlighter.getBestFragment(tokenStream, text));
		...
		TopDocs docs = s.search("content", "美国", 20, 0, null); // hits
...
//		issearcher.setSimilarity(new IKSimilarity());
	...
	public TopDocs search(String[] fields, String keyword, int pageSize, int lastIndex, ScoreDoc lastDoc) throws Exception {
...
//		IKQueryParser.setMaxWordLength(true);
		MultiFieldQueryParser multiParser = new MultiFieldQueryParser(Version.LUCENE_35, fields, analyzer);
//		QueryParser qp = new QueryParser(Version.LUCENE_35, field, analyzer);
		multiParser.setDefaultOperator(QueryParser.OR_OPERATOR);
		...
		if (lastIndex == 0)
			...
			if (null != lastDoc)
				...
				lastDoc = issearcher.search(query, lastIndex).scoreDocs[lastIndex-1];
				hits = issearcher.searchAfter(lastDoc, query, pageSize);
			}
//			hits = issearcher.search(query, lastIndex+pageSize);
	...
	public TopDocs search(String field, String keyword, int pageSize, int lastIndex, ScoreDoc lastDoc, String searchMethod, String start, String end) throws Exception {
		TopDocs hits = null;
		Query query = getQuery(searchMethod, field, keyword, start, end);
	...
	public Query getQuery(String searchMethod, String field, String keyword, String start, String end) {
		Query query = null;
		if (searchMethod.equals("WildCard")) {
			query = new WildcardQuery(new Term(field, keyword));
		} else if (searchMethod.equals("Fuzzy")) {
			query = new FuzzyQuery(new Term(field, keyword), 0.1f, 0);
		} else if (searchMethod.equals("Prefix")) {
			query = new PrefixQuery(new Term(field, keyword));
		} else if (searchMethod.equals("TermByRange")) {
//			System.out.println("TermByRange调用");
			query = new TermRangeQuery(field, start, end, true, true);
		} else if (searchMethod.equals("NumericRange")) {
//			System.out.println("NumericRange调用");
			query = NumericRangeQuery.newIntRange(field, Integer.parseInt(start), Integer.parseInt(end),
					true, true);
		}
		return query;
	}
	
	public Query getQuery(String[] fields, String keyword) throws ParseException {
		MultiFieldQueryParser multiParser = new MultiFieldQueryParser(Version.LUCENE_35, fields, analyzer);
		multiParser.setDefaultOperator(QueryParser.OR_OPERATOR);
		System.out.println("getQuery keyword: " + keyword);
		Query query = multiParser.parse(keyword);
		return query;
	}
	// search函数重载
	public TopDocs search(String field, String keyword, int pageSize, int lastIndex, ScoreDoc lastDoc) throws Exception {
		
		// 直接调用已有方法完成任务
		return search(new String[]{field}, keyword, pageSize, lastIndex, lastDoc);
	}
